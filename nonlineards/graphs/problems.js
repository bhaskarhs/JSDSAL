/*
  ...
  breadth first search can be implemented with a graph.
  A breadth first search is a method of iterating a graph, where we explore the nodes in layers. 
  We start at a specific node, which we will call S.
  On each iteration, we look at the adjacent nodes to S, and add them to a queue, we then move to the first adjacent node and explore its adjacencies.
  We repeat this process until we have looked at all of the graph nodes.
  We will start by implementing a queue to use for our algorithm. 
